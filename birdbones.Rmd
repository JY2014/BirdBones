...
model.0 <- glm(new.type ~ 1, data = pca.df, family = binomial)
...
for (name in variables){
  formula = as.formula(paste0("new.type ~ PC4+", name))
  model <- glm(formula, data = pca.df, family = binomial)
  result <- rbind(result, store(model, paste0("1+PC4+", name), 
                                result$deviance[2], 1))
}
# check the comparison
kable(result)
...
# compare to model with 1 more predictor
for (name in variables){
  formula = as.formula(paste0("new.type ~ PC4+PC1+", name))
  model <- glm(formula, data = pca.df, family = binomial)
  result <- rbind(result, store(model, paste0("1+PC4+PC1+", name), 
                                result$deviance[3], 1))
}
# check the comparison
kable(result)
...
# compare to model with 1 more predictor
for (name in variables){
  formula = as.formula(paste0("new.type ~ PC4+PC1+PC3+", name))
  model <- glm(formula, data = pca.df, family = binomial)
  result <- rbind(result, store(model, paste0("1+PC4+PC1+PC3+", name), 
                                result$deviance[4], 1))
}
# check the comparison
kable(result)
...
variables <- variables[variables != "PC10"]
# compare to model with 1 more predictor
for (name in variables){
  formula = as.formula(paste0("new.type ~ PC4+PC1+PC3+PC10+", name))
  model <- glm(formula, data = pca.df, family = binomial)
  result <- rbind(result, store(model, paste0("1+PC4+PC1+PC3+PC10+", name), 
                                result$deviance[5], 1))
}
# check the comparison
kable(result)
...
```{r, include = FALSE}
result <- result[c(1:5, 10), ]
variables <- variables[variables != "PC8"]
# compare to model with 1 more predictor
for (name in variables){
  formula = as.formula(paste0("new.type ~ PC4+PC1+PC3+PC10+PC8+", name))
  model <- glm(formula, data = pca.df, family = binomial)
  result <- rbind(result, store(model, paste0("1+PC4+PC1+PC3+PC10+PC8+", name), 
                                result$deviance[6], 1))
}
# check the comparison
kable(result)
...
The final model includes the principal component #1, 3, 4, 8 and 10. 

```{r, warning = FALSE}
# check performance of the final model 
model.log <- glm(new.type ~ PC1+PC3+PC4+PC8+PC10, data = train,
                 family = binomial)
# predict on test set
...
svm.linear <- svm(new.type ~ PC1+PC3+PC4+PC8+PC10, 
...
The performance of linear SVM is already comparable to logistic regression. It suggest that the decision boundary is actually linear in this case. However, let's try a radial kernel to see if the accuracy can be even better.

**Radial Basis Kernel**
```{r, cache=TRUE, fig.width=6, fig.height=6}
set.seed(322)
# perform SVM with RBF kernel using default 10-fold CV
svm.rbf <- tune(svm, new.type ~ PC1+PC3+PC4+PC8+PC10, 
